...
func start() int {
	...
	filSystem := os.DirFS("storage/migrations")
	switch os.Args[1] {
	case "up":
		err = migrate.Up(context.Background(), db.DB.DB, filSystem)
	case "down":
		err = migrate.Down(context.Background(), db.DB.DB, filSystem)
	case "to":
		err = migrate.To(context.Background(), db.DB.DB, filSystem, os.Args[2])
	default:
		log.Error("Unkown command", zap.String("name", os.Args[1]))
		return 1
	}
	if err != nil {
